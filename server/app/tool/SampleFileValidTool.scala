...
class SampleFileValidTool(lines: List[List[String]]) {
  ...
  def validHeadersRepeat = {
    ...
    Validated.cond(!valid, true, s"样品信息配置文件表头 ${repeatHeaders.head} 重复!")
  }

  def validHeadersSize = {
    val valid = headers.size < 4
    Validated.cond(!valid, true, s"样品信息配置文件列数必须大于等于4!")
  }
  ...
  def validHeadersExist = {
    ...
    Validated.cond(valid, true, s"样品信息配置文件表头 ${noExistHeaders.head} 不存在!")
  }
  ...
  def validFactorColumn = {
    ...
    val info = factorHeaders.map { header =>
      ...
      val inMessage = if (!inValid) {
        ...
        s"样品信息配置文件第${i + 2}行第${j + 1}列只能为(${factorMap(header).mkString("、")})中的一个!"
      } else ""
      (inValid, inMessage)
    }
    val valid = info.forall(_._1)
  ...
  def validFileNameColumn(fileNames: List[String]) = {
    ...
    val inMessage = if (!inValid) {
      ...
      val j = headers.indexOf(header)
      val i = totalColumns.lastIndexOf(value)
      s"样品信息配置文件第${i + 2}行第${j + 1}列文件名不存在!"
    } else ""
  ...
  def validBatch = {
    ...
    val info = batchs.map { case (batch, batchDatas) =>
      ...
      val j = headers.indexOf(header)
      val inMessage = if (b1) {
        s"样品信息配置文件batch${batch}标样不够(至少两个)!"
      } else if (b2) {
        s"样品信息配置文件batch${batch}待测样不够(至少1个)!"
      } else ""
      (!b1 && !b2, inMessage)
    }
    val valid = info.forall(_._1)
